...
class SwingGui(controller: Controller, instanz : Boolean) extends Frame with Observer:
  ...
  override def update(e: util.Event): Unit = e match {
    ...
    case Event.Mill =>
      ...
      gameStandLabel.text  = 
        s"${controller.gamefield.player.name}, delete a stone."
  ...
  private def updatePlayerStats(): (Panel, Panel, Panel, Panel) = 
    ...
    (showStones.createImagePanel(playerStatistics._1, Color.white),
      showStones.createImagePanel(
        controller.gamesize - (playerStatistics._2 + playerStatistics._1), Color.white),
      showStones.createImagePanel(playerStatistics._3, Color.black),
      showStones.createImagePanel(
        controller.gamesize - (playerStatistics._4 + playerStatistics._3),Color.black))

  private def createMenuBar(): MenuBar =
    val settingsMenu = new Menu("Settings")
    val settingsMenuItem = new MenuItem(Action("Open Settings") {
      new SettingsDialog(controller).visible = true
    })
    settingsMenu.contents += settingsMenuItem
    val menuBar = new MenuBar
    menuBar.contents += settingsMenu
    menuBar
