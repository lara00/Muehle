...
case class Field(
  ...
  private def performAction(number: Int, value: Stone)(
      check: Stone => Boolean
  )(action: (Field, Stone) => Field): Field =
    fields
      .get(number)
      .filter(check)
      .map(stone => action(this, stone))
      .getOrElse(this)

  def setStone(number: Int, value: Stone): Field =
    performAction(number, value)(_ == Stone.Empty) { (field, _) =>
      field.copy(fields = field.fields.updated(number, value))
    }

  def deleteStone(number: Int, value: Stone): Field =
    performAction(number, value)(_ == value) { (field, _) =>
      field.copy(fields = field.fields.updated(number, Stone.Empty))
    }
  ...
  def movestone(from: Int, to: Int, value: Stone): Field =
    performAction(from, value)(_ == value && fields(to) == Stone.Empty) {
      (field, stone) =>
        val updatedField =
          field.copy(fields = field.fields.updated(from, Stone.Empty))
  ...
  def isFieldValid(intValueString: String): Boolean =
    Try(intValueString.toInt).toOption.exists(intValue =>
      (1 to 24).contains(intValue)
    )

  def getStonePositions(stone: Stone): List[Int] =
    fields.filter { case (_, s) => s == stone }.keys.toList

  def getWhiteStonePositions: List[Int] =
    getStonePositions(Stone.White)

  def getBlackStonePositions: List[Int] =
    getStonePositions(Stone.Black)

  private def formatRow(fields: String*): String =
    fields.map(_.padTo(5, ' ')).mkString(" | ") + "\n"
